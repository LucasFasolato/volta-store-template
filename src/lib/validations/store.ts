...
export const storeContentSchema = z.object({
  ...
  hero_image_layout: z.enum(['side', 'background']),
  hero_overlay_opacity: z.number().int().min(20).max(80),
  hero_title: z
    .string()
...
export const storeThemeSchema = z
  .object({
    ...
    heading_font: z.enum(['geist', 'plus-jakarta', 'playfair']),
    body_font: z.enum(['geist', 'plus-jakarta', 'playfair']),
    font_family: z.enum(['geist', 'plus-jakarta', 'playfair']),
