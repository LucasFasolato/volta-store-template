...
export function TrustBar({ store, content, productCount, categoryCount }: { store: Store; content: Pick<StoreContent, 'banner_mode' | 'banner_speed'>; productCount: number; categoryCount: number }) {
  ...
  return <div className="border-y" style={{ borderColor: 'var(--store-card-border)', background: 'linear-gradient(180deg, color-mix(in srgb, var(--store-surface) 62%, transparent), color-mix(in srgb, var(--store-bg) 92%, transparent))' }}>
    {isAnimated ? <div className="overflow-hidden" style={{ maskImage: 'linear-gradient(to right, transparent, black 8%, black 92%, transparent)' }}><div className="store-marquee-track" style={{ ['--marquee-duration' as string]: duration }}><TrustItemsRow items={display} /><TrustItemsRow items={display} ariaHidden /></div></div> : <div className="flex items-stretch overflow-x-auto"><div className="mx-auto flex min-w-max items-stretch"><TrustItemsRow items={display} staticRow /></div></div>}
  </div>
}
