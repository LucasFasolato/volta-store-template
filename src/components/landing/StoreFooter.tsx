import { Clock3, Instagram, MapPin, MessageCircle, Package, Rows3 } from 'lucide-react'
  ...
  return (
    <footer className="border-t" style={{ borderColor: 'var(--store-card-border)', background: 'var(--store-footer-bg-gradient)' }}>
      <div className={cn('mx-auto px-4 py-8 sm:px-6 sm:py-11', containerClass)}>
        <div
          className="overflow-hidden rounded-[calc(var(--store-card-radius)*0.86)] border"
          style={{
            borderColor: 'var(--store-card-border)',
            background: 'linear-gradient(145deg, color-mix(in srgb, var(--store-surface) 94%, transparent), color-mix(in srgb, var(--store-bg) 88%, transparent))',
        ...
        >
          <div className="grid gap-6 p-5 sm:p-6 md:grid-cols-[minmax(0,1.15fr)_minmax(260px,.85fr)] md:items-end">
            ...
            <div className="flex flex-col gap-2 sm:flex-row md:justify-end">
              {phone ? (
                <a href={`https://wa.me/${phone}`} target="_blank" rel="noopener noreferrer" className="store-button inline-flex min-h-11 items-center justify-center gap-2 px-4 text-sm font-semibold transition hover:-translate-y-0.5" style={{ background: 'var(--store-primary)', color: 'var(--store-primary-contrast)' }}>
                  <MessageCircle className="size-4" /> WhatsApp
                </a>
              ) : null}
              {instagram ? (
                <a href={`https://instagram.com/${instagram}`} target="_blank" rel="noopener noreferrer" className="store-button inline-flex min-h-11 items-center justify-center gap-2 border px-4 text-sm font-semibold transition hover:-translate-y-0.5" style={{ borderColor: 'var(--store-card-border)', color: 'var(--store-text)', background: 'color-mix(in srgb, var(--store-surface) 82%, transparent)' }}>
                  <Instagram className="size-4" style={{ color: 'var(--store-primary)' }} /> @{instagram}
                </a>
              ) : null}
            </div>
          </div>

          <div className="flex flex-col gap-1 border-t px-5 py-4 text-[11px] sm:flex-row sm:items-center sm:justify-between sm:px-6" style={{ borderColor: 'var(--store-card-border)', color: 'var(--store-muted-text)' }}>
            <span>Pedido directo al negocio · pago y entrega coordinados por WhatsApp.</span>
            <span className="font-semibold tracking-[0.08em]">VOLTA STORE</span>
          </div>
        </div>
      </div>
    </footer>
