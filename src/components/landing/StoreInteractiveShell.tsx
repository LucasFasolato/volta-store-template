...
export function StoreInteractiveShell({
  ...
  return (
    <>
      ...
      />
      {selectedProduct ? <StoreProductModalClient closeHref={closeModalHref} product={selectedProduct} storeName={storeName} whatsapp={whatsapp} /> : null}
    </>
  )
}
...
function StoreProductModalClient({ closeHref, product, storeName, whatsapp }: { closeHref: string; product: ProductWithImages; storeName: string; whatsapp: string }) {
  ...
  return <ProductModal product={product} storeName={storeName} whatsapp={whatsapp} onClose={() => router.replace(closeHref, { scroll: false })} />
}
