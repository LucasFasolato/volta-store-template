...
export function AdminSidebar({ storeName }: { storeName: string }) {
  ...
  return <aside className="admin-sidebar fixed inset-y-0 left-0 z-30 hidden w-[232px] p-3 lg:block"><div className="flex h-full flex-col overflow-hidden rounded-[18px] border border-white/8 bg-[#10161d] text-white shadow-xl">
    <div className="px-5 pb-4 pt-5"><Link href="/admin" className="admin-sidebar-brand inline-flex items-center gap-2.5"><span className="flex size-7 items-center justify-center rounded-[8px] bg-[#12e89a] text-[11px] font-black text-[#062117]">V</span><span className="text-[15px] font-semibold">VOLTA</span></Link></div>
