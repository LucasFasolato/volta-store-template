...
export function AdminSidebar({ storeName }: AdminSidebarProps) {
  ...
  return (
    <aside className="admin-sidebar fixed inset-y-0 left-0 z-30 hidden w-[232px] p-3 lg:block">
      <div className="flex h-full flex-col overflow-hidden rounded-[18px] border border-white/8 bg-[#10161d] text-white shadow-[0_18px_48px_rgba(15,23,42,.14)]">
        <div className="px-5 pb-4 pt-5">
          <Link href="/admin" className="admin-sidebar-brand inline-flex items-center gap-2.5" aria-label="Ir al resumen">
          ...
          {ADMIN_NAV_ITEMS.map((item) => {
            ...
            return (
              <Link
                ...
                className={cn(
                  'admin-sidebar-nav-link group flex h-10 items-center gap-3 rounded-[9px] px-3 text-[13px] font-medium transition duration-150',
                  active
        ...
        <div className="px-3 pb-3">
          ...
          <div className="border-t border-white/8 pt-3">
            <div className="mb-2 px-2">
              <p className="admin-sidebar-store-name truncate text-[12px] font-medium text-white/90">{storeName}</p>
              <p className="admin-sidebar-store-meta mt-0.5 text-[10px] text-white/38">Administración</p>
            </div>
            <ThemeToggle variant="sidebar" />
            <form action={signOut}>
              <button
                type="submit"
                className="admin-sidebar-signout mt-1 flex h-9 w-full items-center gap-2.5 rounded-[8px] px-2.5 text-[12px] text-white/46 transition hover:bg-white/[0.05] hover:text-red-300"
              >
