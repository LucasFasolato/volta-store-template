...
export function BillingPage({ storeName, overview }: { storeName: string; overview: BillingOverview }) {
  ...
  const statusCopy = status ? STATUS_COPY[status] : STATUS_COPY.not_started
    ...
    <div className="volta-admin-page p-3.5 sm:p-5 lg:p-6">
      <div className="mx-auto max-w-5xl space-y-5">
        ...
        <section className="overflow-hidden rounded-[20px] border border-black/8 bg-white shadow-[0_18px_48px_rgba(15,23,42,.06)] dark:border-white/10 dark:bg-[#111820] dark:shadow-none">
          <div className="grid lg:grid-cols-[1.12fr_.88fr]">
            <div className="p-5 sm:p-7 lg:p-8">
              <div className="flex items-center gap-2 text-[#0e9f6e] dark:text-[#12e89a]">
                <Sparkles className="size-4" />
                <span className="text-xs font-semibold uppercase tracking-[0.16em]">Precio de lanzamiento</span>
              </div>
              <div className="mt-4 flex flex-wrap items-end gap-x-3 gap-y-1">
                <span className="text-4xl font-semibold tracking-[-0.045em] text-foreground sm:text-5xl">{formatBillingAmount(VOLTA_BILLING_PLAN.introAmount)}</span>
                <span className="pb-1 text-sm font-medium text-muted-foreground">/ mes</span>
              </div>
              ...
              <div className="mt-6 grid gap-2.5 sm:grid-cols-3">
                {['Todo VOLTA incluido', 'Cobro mensual automático', 'Cancelás cuando quieras'].map((item) => (
                  <div key={item} className="flex items-center gap-2 rounded-[12px] bg-black/[0.025] px-3 py-2.5 text-xs font-medium text-foreground dark:bg-white/[0.045]">
                    <Check className="size-3.5 shrink-0 text-[#0e9f6e] dark:text-[#12e89a]" />
                    {item}
                  </div>
                ))}
              </div>
              ...
              {!overview.providerConfigured ? (
                <div className="mt-5 rounded-[12px] border border-amber-200 bg-amber-50 px-4 py-3 text-xs leading-5 text-amber-800 dark:border-amber-400/20 dark:bg-amber-400/10 dark:text-amber-200">
                  La activación está temporalmente deshabilitada mientras terminamos la conexión de cobros.
                </div>
              ) : null}

              {subscription?.lastError ? (
                <div className="mt-5 rounded-[12px] border border-red-200 bg-red-50 px-4 py-3 text-xs leading-5 text-red-800 dark:border-red-400/20 dark:bg-red-400/10 dark:text-red-200">{subscription.lastError}</div>
              ) : null}

              <div className="mt-6">
                <BillingActions status={status} providerConfigured={overview.providerConfigured} />
              </div>
            </div>
            ...
            <div className="border-t border-black/7 bg-[#f8faf9] p-5 dark:border-white/8 dark:bg-white/[0.025] sm:p-7 lg:border-l lg:border-t-0 lg:p-8">
              ...
              <div className="mt-4 space-y-4">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <p className="text-xs text-muted-foreground">Importe actual</p>
                    <p className="mt-1 text-2xl font-semibold tracking-tight text-foreground">{formatBillingAmount(currentAmount)}</p>
                  </div>
                  <CreditCard className="size-5 text-[#0e9f6e] dark:text-[#12e89a]" />
                </div>

                <div className="rounded-[14px] border border-black/7 bg-white p-4 dark:border-white/8 dark:bg-white/[0.035]">
                  <div className="flex items-center justify-between gap-3">
                    <p className="text-xs font-semibold text-foreground">Primeros 3 meses</p>
                    <span className="text-xs font-semibold text-muted-foreground">{Math.min(introPaid, introTotal)} / {introTotal}</span>
                  </div>
                  <div className="mt-3 grid grid-cols-3 gap-2">
                    {Array.from({ length: introTotal }).map((_, index) => {
                      const done = index < introPaid || introCompleted
                      return <div key={index} className={`h-2 rounded-full ${done ? 'bg-[#12e89a]' : 'bg-black/8 dark:bg-white/10'}`} />
                    })}
                  </div>
                ...
                <div className="space-y-3 text-sm">
                  <div className="flex items-center justify-between gap-4 border-b border-black/6 pb-3 dark:border-white/7">
                    <span className="flex items-center gap-2 text-muted-foreground"><CalendarDays className="size-4" />Próximo cobro</span>
                    <span className="text-right font-medium text-foreground">{nextCharge || 'A confirmar'}</span>
                  </div>
                  <div className="flex items-center justify-between gap-4">
                    <span className="flex items-center gap-2 text-muted-foreground"><ShieldCheck className="size-4" />Procesador</span>
        ...
        <section className="grid gap-4 md:grid-cols-[1fr_1fr]">
          <div className="rounded-[16px] border border-black/8 bg-white p-5 dark:border-white/10 dark:bg-[#111820]">
            <div className="flex items-center gap-2"><BadgeCheck className="size-4 text-[#0e9f6e] dark:text-[#12e89a]" /><h2 className="text-sm font-semibold text-foreground">Qué incluye</h2></div>
            <p className="mt-3 text-sm leading-6 text-muted-foreground">Catálogo, tienda pública, personalización, pedidos por WhatsApp, analítica y las mejoras que se incorporen al producto. Sin módulos pagos separados.</p>
          </div>
          <div className="rounded-[16px] border border-black/8 bg-white p-5 dark:border-white/10 dark:bg-[#111820]">
            ...
            <p className="mt-3 text-sm leading-6 text-muted-foreground">VOLTA no guarda datos de tarjeta. La autorización y los cobros recurrentes se realizan en Mercado Pago.</p>
          </div>
        </section>

        <section className="rounded-[16px] border border-black/8 bg-white p-5 dark:border-white/10 dark:bg-[#111820] sm:p-6">
          <div className="flex items-end justify-between gap-4">
            <div><p className="admin-label">Movimientos</p><h2 className="mt-1 text-lg font-semibold text-foreground">Últimos cobros</h2></div>
            <span className="text-xs text-muted-foreground">Hasta 8 movimientos</span>
          </div>
          {overview.payments.length ? (
              ...
              {overview.payments.map((payment) => (
                <div key={payment.id} className="flex items-center justify-between gap-4 py-3.5">
                  <div className="min-w-0">
                    <p className="text-sm font-medium text-foreground">{formatBillingAmount(payment.amount)}</p>
            ...
            <div className="mt-4 rounded-[12px] bg-black/[0.025] px-4 py-5 text-sm text-muted-foreground dark:bg-white/[0.035]">Todavía no hay cobros registrados.</div>
          )}
        </section>
      </div>
    </div>
