  ...
  if found then
    ...
    set status = 'creating',
        ...
        checkout_idempotency_key = case
          when billing_row.status in ('creating', 'pending') and billing_row.checkout_idempotency_key is not null
            then billing_row.checkout_idempotency_key
          else gen_random_uuid()
        end,
        current_amount = case
          when billing_row.intro_cycles_paid >= billing_row.intro_cycles_total then billing_row.standard_price
          else billing_row.intro_price
        end,
        canceled_at = null,
